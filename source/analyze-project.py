#!/usr/bin/env python3
"""analyze-project — collate the project + an analysis prompt to the clipboard.

Builds a directory tree and XML file contents, prepends a system prompt that
puts the model in read-and-answer mode, and copies the whole thing.
"""
import argparse
import os
...
def main():
    ...
    prompt, included, ignored = collate.build_prompt(
        prompts.ANALYZE_SYSTEM_PROMPT, root=root, include_tree=not args.no_tree,
    ...
    ui.ok(f"Analysis prompt for {len(included)} file(s) copied to clipboard.")
